    ...
    def generate_override(self, separator="."):
        ...
        if not isinstance(data, dict):
            raise YapconfLoadError(
                ...
                "the result was not a dictionary. Got %s" % (self.label, type(data))
            )
    ...
    def _watch(self, handler, data):
        if self.filename:
            file_handler = FileHandler(
                filename=self.filename, handler=handler, file_type="json"
            )
            observer = Observer()
...
class YamlConfigSource(ConfigSource):
    ...
    def _watch(self, handler, data):
        file_handler = FileHandler(
            filename=self.filename, handler=handler, file_type="yaml"
        )
        observer = Observer()
        directory = os.path.dirname(self.filename)
        observer.schedule(file_handler, directory, recursive=False)
...
class KubernetesConfigSource(ConfigSource):
    ...
    def validate(self):
        if yapconf.kubernetes_client and not isinstance(
            self.client, yapconf.kubernetes_client.CoreV1Api
        ):
            raise YapconfSourceError(
                "Invalid source (%s). Client must be supplied and must be of "
                "type %s. Got type: %s"
    ...
    def get_data(self):
        result = self.client.read_namespaced_config_map(self.name, self.namespace)
        if self.key is None:
            return result.data
    ...
    def _watch(self, handler, data):
        ...
        for event in w.stream(
            self.client.list_namespaced_config_map, namespace=self.namespace
        ):
